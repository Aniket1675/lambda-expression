...
public class UserRegistration {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(isValidFirstName.validate("^[A-Z]{1}[a-z]{2,}$", "Aniket"));

        Validation isValidLastName = (pattern, lastName) -> {
            return "Last Name is " + Pattern.compile(pattern).matcher(lastName).matches();
        };
        System.out.println(isValidLastName.validate("^[A-Z]{1}[a-z]{2,}$", "Kumbhar"));
        ...
        System.out.println(isValidMobileNumber.validate("^[91]+[ ]?[6-9]{1}[0-9]{9}$", "91 7020409312"));

        Validation isValidPassword1 = (pattern, password) -> {
            return "Password rule is " + Pattern.compile(pattern).matcher(password).matches();
        };
        System.out.println(isValidPassword1.validate("^[0-9a-zA-Z!,@#$&*().]{8,}$", "Aniket@123"));
